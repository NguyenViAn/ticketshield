...
const CARD_STYLES: Record<TicketTierId, { base: string; glow: string }> = {
  VIP: {
    base: "border-emerald-400/18 bg-[linear-gradient(180deg,rgba(7,26,19,0.98),rgba(4,16,12,0.98))]",
    glow: "shadow-[0_0_30px_-8px_rgba(52,211,153,0.35),0_22px_60px_-42px_rgba(0,0,0,0.6)]",
  },
  Premium: {
    base: "border-cyan-400/14 bg-[linear-gradient(180deg,rgba(7,24,26,0.96),rgba(5,17,19,0.98))]",
    glow: "shadow-[0_0_30px_-8px_rgba(34,211,238,0.3),0_22px_60px_-42px_rgba(0,0,0,0.6)]",
  },
  Standard: {
    base: "border-white/10 bg-[linear-gradient(180deg,rgba(8,24,19,0.96),rgba(5,17,13,0.96))]",
    glow: "shadow-[0_0_26px_-8px_rgba(148,163,184,0.2),0_22px_60px_-42px_rgba(0,0,0,0.6)]",
  },
  Economy: {
    base: "border-white/10 bg-[linear-gradient(180deg,rgba(8,24,19,0.92),rgba(5,17,13,0.94))]",
    glow: "shadow-[0_0_22px_-8px_rgba(100,116,139,0.2),0_22px_60px_-42px_rgba(0,0,0,0.6)]",
  },
};
  ...
  if (isLoading) {
    return (
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[1, 2, 3, 4].map((item) => (
          <div key={item} className="h-[300px] animate-pulse rounded-[24px] bg-white/5" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div id="seat-tier-grid" className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {TICKET_TIERS.map((tier) => {
        ...
        const availablePercent = Math.round((available / tier.totalSeats) * 100);
        const styles = CARD_STYLES[tier.id];
        const accent = TIER_ACCENT[tier.id];

        return (
          <motion.div
            key={tier.id}
            whileHover={!isSoldOut ? { y: -4, scale: 1.01 } : undefined}
            transition={{ type: "spring", stiffness: 300, damping: 24 }}
          >
            <div
              className={`flex min-h-[300px] h-full flex-col rounded-[26px] border px-5 pb-5 pt-4 transition-all duration-300 ${
                isSoldOut
                  ? "border-white/8 bg-white/[0.04] opacity-65"
                  : `${styles.base} shadow-[0_22px_60px_-42px_rgba(0,0,0,0.6)]`
              } ${isSelected ? `ring-2 ring-emerald-400/50 ${styles.glow}` : ""}`}
            >
              <div className="flex items-start justify-between gap-3">
                <span
                  className={`inline-flex min-h-8 items-center gap-1.5 rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] ${
                    isSelected
              ...
              <div className="mt-3">
                ...
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/8">
                  <motion.div
                    className={`h-full rounded-full ${isSoldOut ? "bg-slate-600" : accent}`}
                    initial={{ width: "0%" }}
                    animate={{ width: `${availablePercent}%` }}
                    transition={{ duration: 0.8, ease: "easeOut", delay: 0.1 }}
                    style={{ opacity: isSoldOut ? 0.4 : 0.85 }}
                  />
                </div>
              </div>

              <div className="theme-inset-surface mt-5 rounded-[22px] px-4 py-4">
