  ...
  return (
    <div className="space-y-3 p-4 sm:space-y-4 sm:p-5 lg:p-6">
      {events.map((event, index) => {
        ...
        return (
          <div key={event.id} className="group relative pl-12 sm:pl-16">
            {index < events.length - 1 ? (
              <span className="absolute left-[18px] top-10 h-[calc(100%-1rem)] w-px bg-gradient-to-b from-cyan-500/20 via-white/6 to-transparent sm:left-[22px] sm:top-12" />
            ) : null}

            <div className={`absolute left-0 top-0 flex h-9 w-9 items-center justify-center rounded-xl border sm:h-11 sm:w-11 sm:rounded-2xl ${
              meta.tone === "emerald"
            ...
            <div className="rounded-[18px] border border-white/6 bg-[linear-gradient(180deg,rgba(24,30,39,0.98),rgba(20,25,34,0.98))] p-3 transition-colors group-hover:border-cyan-500/10 sm:rounded-[24px] sm:p-4">
